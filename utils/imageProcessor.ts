...
export async function processImages(
  ...
  // 이미지 처리 작업 목록 생성
  const processingTasks = matches.map(async (match) => {
    const [fullMatch, altText, imagePath] = match;

    // 로컬 이미지 경로인 경우에만 처리
    if (imagePath.startsWith("http") || imagePath.startsWith("data:")) {
      return { fullMatch, newUrl: imagePath };
    }
    ...
    try {
      ...
      if (publicUrl) {
        ...
        return { fullMatch, newUrl: publicUrl };
      }
    } catch (error) {
      console.error(
        `이미지 처리 실패 (${imagePath}):`,
  ...
  for (const { fullMatch, newUrl } of results) {
    if (fullMatch) {
      const [, altText] = fullMatch.match(/!\[(.*?)\]\((.*?)\)/) || [null, ""];
      processedContent = processedContent.replace(
        fullMatch,
        `![${altText}](${newUrl})`
      );
