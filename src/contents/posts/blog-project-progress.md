...
하지만 Next는 처음부터 폴더구조를 제대로 잡고 시작해야하기 때문에, 최적화 관점에서는 Next가 훨씬 편하다는 생각을 할 수 밖에 없었다. 컴포넌트도 사실상 "강제로" 서버컴포넌트와 클라이언트 컴포넌트로 나누어야 한다. 클라이언트 컴포넌트 내에서도 쉽게 JS를 분리하는 작업이 가능해서 추후의 디버깅이나 최적화를 하는 것이 편하게 다가왔다.
