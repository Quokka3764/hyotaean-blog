...
type Point = string | { text: string; href: string };

const projectItems: { title: string; points: Point[] }[] = [
  {
    title: "프로젝트 기획 및 총괄",
    points: [
  ...
  },
  {
    ...
    points: [
      ...
      { text: "왜 JWT 인가?", href: "/blog/finalProject-refactor" },
      { text: "PKCE 구현 상세", href: "/blog/finalProject-PKCE" },
      { text: "PKCE 구현 정리", href: "/blog/finialProjectLogin-post" },
    ],
  },
  {
...
const ProjectItemCard = ({
  ...
  item: { title: string; points: Point[] };
}) => (
  <div key={item.title}>
    ...
    <ul className="list-disc pl-5 text-gray-600 dark:text-gray-400 space-y-1.5 text-sm leading-relaxed">
      {item.points.map((point, index) => {
        if (typeof point === "string") {
          const isLastTextBeforeLinks =
            item.title === "JWT 기반 소셜 로그인 구축" &&
            typeof item.points[index + 1] === "object";
          return (
            <li key={index} className={isLastTextBeforeLinks ? "mb-1" : ""}>
              {point}
            </li>
          );
        } else if (item.title === "JWT 기반 소셜 로그인 구축") {
          const isFirstLink = typeof item.points[index - 1] === "string";
          if (isFirstLink) {
            return (
              <li key="link-group" className="!list-none -ml-5 pt-1">
                <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 mr-2">
                  관련 글:
                </span>
                {item.points.slice(index).map(
                  (linkPoint, linkIndex) =>
                    typeof linkPoint === "object" && (
                      <React.Fragment key={linkPoint.href + linkIndex}>
                        <Link
                          href={linkPoint.href}
...
export default function AboutPage() {
  const leftColumnItems = projectItems.filter((_, index) => index % 2 === 0);
  const rightColumnItems = projectItems.filter((_, index) => index % 2 !== 0);
