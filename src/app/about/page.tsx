...
//   const items : {title: string; points: Point[]}[]
//   return (
...
//                   {text: "PKCE를 위해서 프론트가 한 작업", href:"http://localhost:3000/blog/finalProject-PKCE" }
...
const projectItems: { title: string; points: Point[] }[] = [
  ...
  },
  {
    ...
    points: [
      "보안성을 고려한 토큰(Access/Refresh) 저장 전략 설계 (메모리/HttpOnly 쿠키)",
      "토큰 자동 갱신 로직 구현으로 사용자 세션 유지 및 로그인 요청 최소화",
      "프로젝트가 끝난 후, 추가 보안을 위해 PKCE 도입", // 링크 그룹 전 마지막 텍스트 항목
      { text: "왜 JWT 인가?", href: "/blog/finalProject-refactor" }, // 링크 텍스트에서 '관련 글:' 제거
      { text: "PKCE 도입 배경", href: "/blog/finalProject-PKCE" },
                  ...
                  {item.points.map((point, index) => {
                    if (typeof point === "string") {
                      ...
                      return (
                        <li
                          key={index}
                          className={isLastTextBeforeLinks ? "mb-1" : ""}
                        >
                          {point}
                        </li>
                      );
                      ...
                      const isFirstLink =
                        typeof item.points[index - 1] === "string";
                      if (isFirstLink) {
                        return (
                          <li
                            key="link-group"
                            className="!list-none -ml-5 pt-1"
                          >
                    ...
                    // 다른 섹션에 링크 객체가 있을 경우를 대비한 기본 처리 (렌더링 안 함)
                    return null;
                  })}
                </ul>
              </div>
